...
(function (global) {
  ...
  const CSS = `
    ...
    .bbchat-header-title-wrap { display: flex; align-items: center; gap: 8px; }
    .bbchat-header-title-wrap svg { width: 28px; height: 28px; flex-shrink: 0; }
    ...
    .dp-day.dp-booked { position: relative; color: #C77A00; }
    .dp-day.dp-booked::after {
      content: ''; position: absolute; bottom: 3px; left: 50%;
      transform: translateX(-50%); width: 4px; height: 4px;
      border-radius: 50%; background: #E8A317;
    }
    .dp-day.dp-booked.dp-selected { color: white; }
    .dp-day.dp-booked.dp-selected::after { background: white; }
    .dp-booked-note {
      margin-top: 8px; padding: 6px 9px; border-radius: 8px;
      background: #FFF6E5; border: 1px solid #F0D08A; color: #8A5A00;
    ...
    return `
      ...
      <div id="bbchat-container">
        <div class="bbchat-header" id="bbchat-header">
          <div class="bbchat-header-title-wrap">
  ...
  BBChatWidget.prototype._addMessage = function (text, sender, files, buttons) {
    ...
    // [datepicker] may carry booked dates as a payload, e.g. [datepicker:2026-07-04,2026-07-11]
    const dpMatch = (sender === 'bot' && text) ? text.match(/\[datepicker(?::([^\]]*))?\]/i) : null;
    const hasDatePicker = !!dpMatch;
    const bookedDates = (dpMatch && dpMatch[1])
      ? dpMatch[1].split(',').map(s => s.trim()).filter(Boolean)
      : [];
    ...
    if (hasDatePicker) {
      ...
      this._renderDatePicker(dpContainer, bookedDates);
    }
  ...
  // ─── INLINE DATE PICKER ────────────────────────────────────────────────────
  BBChatWidget.prototype._renderDatePicker = function (anchorEl, bookedDates) {
    const self  = this;
    ...
    // Booked dates are a suggestion only — they are marked but stay selectable.
    const bookedSet = new Set(Array.isArray(bookedDates) ? bookedDates : []);
    ...
    function build() {
      ...
      for (let d = 1; d <= daysInMonth; d++) {
        ...
        if (cellDate < todayMid) {
          ...
          if (bookedSet.has(ymd(cellDate))) btn.classList.add('dp-booked');
      ...
      // Gentle heads-up if the chosen day is one BBK marked as booked (still allowed).
      if (selected && bookedSet.has(ymd(selected))) {
        const note = document.createElement('div');
        note.className = 'dp-booked-note';
        note.textContent = '📌 This date is already booked, but you can still select it — all orders are subject to BBK\'s approval.';
